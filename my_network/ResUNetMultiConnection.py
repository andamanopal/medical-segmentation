...
class Encoder(nn.Module):
    def __init__(self, in_channels, resnet_config=resnet18_config, pretrained_path=None):
        super().__init__()
...
class Decoder(nn.Module):
    def __init__(self, conv_inchannels, num_features, activation=nn.PReLU):
        super().__init__()
        self.convblock2 = ConvBlock2(in_channels=conv_inchannels, out_channels=num_features, activation=activation)

    def forward(self, x, skip_connections):
        for unscaled_input in skip_connections:
            scale_factor = x.shape[-1] / unscaled_input.shape[-1]
            x = torch.cat([x, nn.functional.interpolate(unscaled_input, scale_factor=scale_factor)], dim=1)
        x = self.convblock2(x)
        return x


class ResUNetMultiConnection(nn.Module):
    def __init__(self, in_channels, out_channels, num_features=32, upsample_mode='bilinear', activation=nn.PReLU,
                 resnet_config=resnet18_config):
        super().__init__()
        self.encoder = Encoder(in_channels=in_channels,
                               resnet_config=resnet_config,
                               pretrained_path='weights/resnet18-pretrained.pth')

        # [64, 128, 256, 512]

        self.dec_conv4 = Decoder(conv_inchannels=num_features+np.sum(resnet_config.channels[:-1])+64,
                                 num_features=num_features,
                                 activation=activation)
        self.dec_conv3 = Decoder(conv_inchannels=num_features+np.sum(resnet_config.channels[:-2])+64,
                                 num_features=num_features,
                                 activation=activation)
        self.dec_conv2 = Decoder(conv_inchannels=num_features+np.sum(resnet_config.channels[:-3])+64,
                                 num_features=num_features,
                                 activation=activation)
        self.dec_conv1 = Decoder(conv_inchannels=num_features + 64,
                                 num_features=num_features,
                                 activation=activation)

        self.up_conv5 = UpConv(in_channels=resnet_config.channels[-1],
                               out_channels=num_features,
                               mode=upsample_mode,
                               activation=activation)
        self.up_conv4 = UpConv(in_channels=num_features,
                               out_channels=num_features,
                               mode=upsample_mode,
                               activation=activation)
        self.up_conv3 = UpConv(in_channels=num_features,
                               out_channels=num_features,
                               mode=upsample_mode,
                               activation=activation)
        self.up_conv2 = UpConv(in_channels=num_features,
                               out_channels=num_features,
                               mode=upsample_mode,
                               activation=activation)
        self.up_conv1 = UpConv(in_channels=num_features,
                               out_channels=num_features,
                               mode=upsample_mode,
                               activation=activation)
    ...
    def forward(self, x):
        y, skip_connections = self.encoder(x)  # 16, [32, 64, 128, 256] size

        dec = self.dec_conv4(self.up_conv5(y), skip_connections[:])    # 32
        dec = self.dec_conv3(self.up_conv4(dec), skip_connections[1:]) # 64
        dec = self.dec_conv2(self.up_conv3(dec), skip_connections[2:]) # 128
        dec = self.dec_conv1(self.up_conv2(dec), skip_connections[3:]) # 256
