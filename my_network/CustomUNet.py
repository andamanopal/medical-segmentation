...
def init_net(net, init_type="normal", init_gain=0.02, device_ids=None):
    ...
    if torch.cuda.is_available():
        ...
        net.to(device_ids[0])
        if len(device_ids) > 1:
            print(f'Initializing DataParallel with device_idz={device_ids}')
...
class CustomUNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_features=32,
                 feature_mode="fixed", pool="avg", upsample_mode="bilinear", activation=nn.PReLU):
        ...
        if feature_mode == "fixed":
            self.enc_conv1 = ConvBlock2(in_channels=num_features, out_channels=num_features, activation=activation)
            self.enc_conv2 = ConvBlock2(in_channels=num_features, out_channels=num_features, activation=activation)
            self.enc_conv3 = ConvBlock2(in_channels=num_features, out_channels=num_features, activation=activation)
            self.enc_conv4 = ConvBlock2(in_channels=num_features, out_channels=num_features, activation=activation)
            ...
            self.dec_conv4 = ConvBlock2(in_channels=2 * num_features, out_channels=num_features, activation=activation)
            self.dec_conv3 = ConvBlock2(in_channels=2 * num_features, out_channels=num_features, activation=activation)
            self.dec_conv2 = ConvBlock2(in_channels=2 * num_features, out_channels=num_features, activation=activation)
            self.dec_conv1 = ConvBlock2(in_channels=2 * num_features, out_channels=num_features, activation=activation)
        else:
            self.enc_conv1 = ConvBlock2(in_channels=1 * num_features, out_channels=2 * num_features, activation=activation)
            self.enc_conv2 = ConvBlock2(in_channels=2 * num_features, out_channels=4 * num_features, activation=activation)
            self.enc_conv3 = ConvBlock2(in_channels=4 * num_features, out_channels=8 * num_features, activation=activation)
            self.enc_conv4 = ConvBlock2(in_channels=8 * num_features, out_channels=16 * num_features, activation=activation)

            self.up_conv4 = UpConv(in_channels=16 * num_features, out_channels=8 * num_features,mode=upsample_mode, activation=activation)
            self.up_conv3 = UpConv(in_channels=8 * num_features, out_channels=4 * num_features, mode=upsample_mode, activation=activation)
            self.up_conv2 = UpConv(in_channels=4 * num_features, out_channels=2 * num_features, mode=upsample_mode, activation=activation)
            self.up_conv1 = UpConv(in_channels=2 * num_features, out_channels=1 * num_features, mode=upsample_mode, activation=activation)

            self.dec_conv4 = ConvBlock2(in_channels=16 * num_features, out_channels=8 *num_features, activation=activation)
            self.dec_conv3 = ConvBlock2(in_channels=8 * num_features, out_channels=4 * num_features, activation=activation)
            self.dec_conv2 = ConvBlock2(in_channels=4 * num_features, out_channels=2 * num_features, activation=activation)
            self.dec_conv1 = ConvBlock2(in_channels=2 * num_features, out_channels=1 * num_features, activation=activation)
